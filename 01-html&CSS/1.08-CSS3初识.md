...
| `WebKit`内核 | CSS前缀为`"-webkit-"` | 如Chrome、Safari |
...
旧：
 ...
 新增的属性选择器
  ...
  ` E[attr^="val"]` 选择拥有`attr`属性且属性值为`val`开头的E元素

   `E[attr$="val"] `选择拥有`attr`属性且属性值以`val`结束的E元素

   `E[attr*="val"]  `选择拥有`attr`属性且属性值中包含`val`的E元素
...
```html
 	<style>
        ...
        li:nth-last-child(3n){background:red;}
    </style>
<ul>
    <li>1</li>
    <li>2</li>
